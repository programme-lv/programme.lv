...
func RegisterProductRoutes(router *mux.Router) {
	...
	router.HandleFunc("/tasks/{task_id}", controllers.GetTask).Methods("GET")
	...
	router.HandleFunc("/submissions/{subm_id}", controllers.GetSubmission).Methods("GET")

	// languages

	// execute
	router.HandleFunc("/execute/enqueue", controllers.EnqueueExecution).Methods("POST")
	router.HandleFunc("/execute/{execution_id}", controllers.GetExecution).Methods("GET")
}
